...
def get_random_color(base_color: Union[Tuple[int, int, int], str] = BASE_COLOR, weight: float = WEIGHT) -> str:
    """Generate a random color in rgb string format."""
    weight = max(0.0, min(weight, 1.0))
    if isinstance(base_color, str): base_color = tuple(int(base_color.replace('#', '')[i:i + 2], 16) for i in (0, 2, 4))
    return '#' + ''.join(map(lambda x: str(format(round(x * 255), 'x')).rjust(2, '0'), (
